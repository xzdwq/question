...
## Start
 ...
 8. Объяснить что это в JS
    ...
    1) .? - необязательная цепочка вызова - foo?.bar?.fb?.()
    2) ... - спред или рест оператор, чтобы собрать множетсво в одно - [..arr1, ...arr2]
    3) ?? - нулевой оператор использует правое значение, если левый null или undefined
    4) ||= - привоение логического или, присваивает правое значение если левое отвечает логическому false
     ```javascript
     ...
     ```
    5) &&= - привоение логического и, присваивает правое значение если левое отвечает логическому true
     ```javascript
     let a = 1;
     let b = 0;
     ...
     ```
    6) .at(-1) - берет указанный индекс из массива, в случае -1, берет первый с конца и .т.д.
